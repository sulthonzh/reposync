...
export interface ScanResult {
  ...
  timestamp: string;
  originalDriftCount: number;
  filteredDriftCount: number;
}
...
export function scan(repos: RepoInfo[], options: {
  ignorePackages?: string[];
  ignoreFields?: string[];
  severityThreshold?: 'high' | 'medium' | 'low';
} = {}): ScanResult {
  const { ignorePackages = [], ignoreFields = [], severityThreshold = 'low' } = options;
    ...
    for (const depType of ['dependencies', 'devDependencies'] as const) {
      if (pkg[depType]) {
        for (const depName of Object.keys(pkg[depType])) {
          if (ignorePackages.includes(depName)) {
            return false;
          }
        }
      }
  ...
  compareDeps(filteredRepos, 'dependencies', drifts, ignorePackages);
  compareDeps(filteredRepos, 'devDependencies', drifts, ignorePackages);
  ...
  const filteredDrifts = drifts.filter(drift => {
    ...
    return severityOrder[drift.severity] >= thresholdOrder;
  });
  ...
  return { 
    ...
    originalDriftCount: drifts.length,
    filteredDriftCount: filteredDrifts.length
  };
}
...
export function formatDriftTable(result: ScanResult): string {
  ...
  if (result.originalDriftCount !== result.filteredDriftCount) {
    lines.push(`Original drifts: ${result.originalDriftCount}, filtered: ${result.filteredDriftCount}`);
  }
  lines.push('');
...
export function formatJson(result: ScanResult): string {
  return JSON.stringify({
    ...
    originalDriftCount: result.originalDriftCount,
    driftCount: result.filteredDriftCount,
